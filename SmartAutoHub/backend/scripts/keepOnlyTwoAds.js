/**
 * Script to keep only dcxd and Toyota Lanka browse ads
 */
...
async function keepOnlyTwoAds() {
  try {
    ...
    // Find ads to deactivate (all except the 2 to keep)
    const adsToRemove = allBrowseAds.filter(ad => !adIdsToKeep.includes(ad._id.toString()));
    
    if (adsToRemove.length === 0) {
      console.log('✅ No other ads to remove. Only the 2 required ads exist.');
      await mongoose.connection.close();
      process.exit(0);
    }
    ...
    // Close connection
    await mongoose.connection.close();
    console.log('\n✓ Done! Only dcxd and Toyota Lanka browse ads remain active.');
    process.exit(0);
